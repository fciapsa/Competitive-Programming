...
bintree <char> construyeArbol(std::stack<char> &pila) {
	char raiz = pila.top();
	pila.pop();
...
int main() {
	...
	while (std::cin) {
		std::stack<char> pila;
		for (int i = 0; i < s.size(); ++i) pila.push(s[i]);

		bintree<char> arbol = construyeArbol(pila);
		std::vector<char> ordenCola = arbol.levelorder();
		for (int i = ordenCola.size() - 1; i >= 0; --i) {
			std::cout << ordenCola[i];
		}
		std::cout << '\n';
	getline(std::cin, s);
	}
