...
int main() {
	int n, c, o, d, e, p, orig, act, sol;
	bool ok;
	std::cin >> n;
	while (std::cin) {
		std::cin >> c;
		std::vector<std::vector<arista>> adyList(n + 1), invAdyList(n + 1);
		for (int i = 0; i < c; ++i) {
			std::cin >> o >> d >> e;
			adyList[o].push_back({ d,e });
			invAdyList[d].push_back({ o,e });
		}

		std::cin >> orig >> p;
		std::vector<int> destinos(p);
		for (int i = 0; i < p; ++i)std::cin >> destinos[i];
		...
		while (!pq.empty()) {
			...
			for (int i = 0; i < adyList[act].size(); ++i) {
				if (distTo[act] + adyList[act][i].val < distTo[adyList[act][i].dest]) {
					distTo[adyList[act][i].dest] = distTo[act] + adyList[act][i].val;
					pq.update(adyList[act][i].dest, distTo[adyList[act][i].dest]);
				}
			}
		}

		std::vector<int> distFrom(n + 1, INT_MAX);
		distFrom[orig] = 0;
		pq.push(orig, 0);
		while (!pq.empty()) {
			act = pq.top().elem;
			...
			for (int i = 0; i < invAdyList[act].size(); ++i) {
				if (distFrom[act] + invAdyList[act][i].val < distFrom[invAdyList[act][i].dest]) {
					distFrom[invAdyList[act][i].dest] = distFrom[act] + invAdyList[act][i].val;
					pq.update(invAdyList[act][i].dest, distFrom[invAdyList[act][i].dest]);
				}
			}
		}

		ok = true;
		for (int i = 0; i < p && ok; ++i) ok = distTo[destinos[i]] != INT_MAX;
		for (int i = 0; i < p && ok; ++i) ok = distFrom[destinos[i]] != INT_MAX;

		if (ok) {
			sol = 0;
			for (int i = 0; i < p; ++i) sol += (distTo[destinos[i]] + distFrom[destinos[i]]);
			std::cout << sol << '\n';
		}
