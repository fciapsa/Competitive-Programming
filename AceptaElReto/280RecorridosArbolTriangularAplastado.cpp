...
bintree<int> crearArbol(std::vector<int> const&inorden, int ini, int fin) {
	if (ini >= fin) return {};
	if (ini == fin - 1) return { inorden[ini] };
	if (ini == fin - 2) return{ {inorden[ini]},inorden[ini + 1],{} };

	int raiz = (ini+fin) / 2;
	return{ crearArbol(inorden,ini,raiz),inorden[raiz],crearArbol(inorden,raiz + 1,fin) };
}
...
int main() {
	...
	while (x != -1) {
		...
		bintree<int> arb = crearArbol(inorden, 0, inorden.size());
		std::vector<int> pre = arb.preorder();
		std::cout << pre[0];
		for (int i = 1; i < pre.size(); ++i)std::cout << ' ' << pre[i];
		std::cout << '\n';
		std::vector<int> post = arb.postorder();
		std::cout << post[0];
