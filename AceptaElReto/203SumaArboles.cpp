...
protected:
	static void preorder(Link a, std::vector<T> & pre) {
		...
		else pre.push_back(-1);//cambio para el problema
	}
...
bintree<int> sumar(bintree<int> const& arb1, bintree<int> const& arb2) {
	if (arb1.empty() && arb2.empty()) return bintree<int>();
	else if (arb1.empty()) return bintree<int>(arb2.left(), arb2.root(), arb2.right());
	else if (arb2.empty()) return bintree<int>(arb1.left(), arb1.root(), arb1.right());
	else return bintree<int>(sumar(arb1.left(), arb2.left()), arb1.root() + arb2.root(), sumar(arb1.right(), arb2.right()));
}

bool resuelveCaso() {
	bintree<int> arb1 = leerArbol(-1), arb2 = leerArbol(-1);

	if (arb1.empty() && arb2.empty()) return false;

	bintree<int> suma = sumar(arb1,arb2);
	std::vector<int> v = suma.preorder();
