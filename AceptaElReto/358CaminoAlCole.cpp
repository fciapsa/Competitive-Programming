...
void relax(arista const& a, std::vector<int>& distTo, std::vector<int>& formasTo,IndexPQ<int> &pq) {
	int aux = distTo[a.from] + a.peso;
	if (aux < distTo[a.to]) {
		distTo[a.to] = aux;
		pq.update(a.to-1, aux);
		formasTo[a.to] = formasTo[a.from];
	}
	else if (aux == distTo[a.to]) {
		formasTo[a.to] += formasTo[a.from];
	}
}

int main() {
	int N, C, a, b, peso, act;
	std::cin >> N;
	while (std::cin) {
		std::cin >> C;
		std::vector<std::vector<arista>> adyList(N + 1);
		for (int i = 0; i < C; ++i) {
			std::cin >> a >> b >> peso;
			adyList[a].push_back({ a,b,peso });
			adyList[b].push_back({ b,a,peso });
		}

		std::vector<int> formasTo(N + 1, 0);
		formasTo[1] = 1;
		IndexPQ<int> pq(N);
		...
		while (!pq.empty()) {
			act = pq.top().elem;
			++act;//en la pq van de 0 a N-1
			pq.pop();

			for (arista a : adyList[act]) {
				relax(a, distTo, formasTo,pq);
			}
		}
