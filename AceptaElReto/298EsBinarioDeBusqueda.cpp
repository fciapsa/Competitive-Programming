...
struct tTerna {//informacion que necesitamos de los hijos
	bool cumple;
	int max, min;
};

tTerna binDeBusq(bintree<int> const&arbol) {
	if (arbol.empty()) return { true,0,0 };
	else if (arbol.left().empty() && arbol.right().empty()) return { true,arbol.root(),arbol.root() };
	else if (arbol.left().empty()) {
		tTerna der = binDeBusq(arbol.right());
		if (der.cumple) return{ arbol.root() < der.min,std::max(arbol.root(),der.max),std::min(der.min,arbol.root()) };
		else return { false,0,0 };
	}
	else if (arbol.right().empty()) {
		tTerna izq = binDeBusq(arbol.left());
		if (izq.cumple) return{ arbol.root() > izq.max,std::max(izq.max,arbol.root()),std::min(arbol.root(),izq.min) };
		else return { false,0,0 };
	}
	else {
		tTerna izq = binDeBusq(arbol.left());
		...
		if (izq.cumple) {
			...
			if (der.cumple)
				return { arbol.root() > izq.max && arbol.root() < der.min,
				std::max(izq.max,std::max(arbol.root(),der.max)),
				std::min(der.min,std::min(izq.min,arbol.root())) };
			else return { false,0,0 };
		}
		else return{ false,0,0 };
	}
...
void resuelveCaso() {
	...
	if (binDeBusq(arbol).cumple) std::cout << "SI\n";
	else std::cout << "NO\n";
}
...
int main() {
	int nc;
	std::cin >> nc;
	for (int i = 0; i < nc; ++i) resuelveCaso();
	return 0;
}